...
def fetch_vtho_generated(period="day"):
    ...
    while True:
        ...
        if not r.ok:
            return pd.DataFrame(columns=["date", "vtho", "blockNumber"]), f"API error {r.status_code}: {r.text}"
...
# ── Load data (spinner lives here, outside cache) ────────
with st.spinner("Fetching data..."):
    df, error = fetch_vtho_generated(period)
...
# ── Sidebar date picker (needs df loaded first) ──────────
with st.sidebar:
